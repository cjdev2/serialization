...
class ThriftTest extends FlatSpec with Matchers with PropertyChecks {
  ...
  it should "be able to deserialize `TestRecord`s" in {
    forAll { (s: String, n: Int) =>
      ...
      implicit object DeserializeThriftStructTestRecord
  ...
  // TODO: Finish converting the below test to property tests
  ...
  it should "be declarable explicitly rather than implicitly" in {
    ...
    implicit object ImplicitDeserializeThriftStruct
      extends DeserializeThriftStruct[TestRecord](TestRecord)
