...
class YamlSpecTest extends FlatSpec with Matchers {
  ...
  it should "correctly parse sequence of scalars" in {
    ...
    val expected: Result[Yaml] = Result.safely(Yaml.arr(
      "Mark McGwire",
      "Sammy Sosa",
      "Ken Griffey"
    ))
    ...
    // when
    val actual: Result[Yaml] = Yaml.parse(raw)
  ...
  it should "correctly parse mapping scalars to scalars" in {
    ...
    val expected: Result[Yaml] = Result.safely(Yaml.obj(
      "hr" -> 65,
      "avg" -> 0.278,
      "rbi" -> 147
    ))
  ...
  it should "correctly parse two documents in a stream" in {
    ...
    val expected: Result[Yaml] =
      Result.safely(Yaml.stream(Stream(
        Yaml.arr(
          "Mark McGwire",
          "Sammy Sosa",
          "Ken Griffey"
  ...
  ignore should "correctly parse play by play feed from a game" in {
    ...
    val expected: Result[Yaml] = Result.safely(Yaml.stream(Stream(
      Yaml.obj(
        "time" -> "20:03:20",
        "player" -> "Sammy Sosa",
        "action" -> "strike (miss)"
  ...
  ignore should "correctly parse Mapping between Sequences" in {
    ...
    val expected: Result[Yaml] = Result.safely(Yaml.assoc(Map(
  ...
  it should "correctly parse In the folded scalars, newlines become spaces" in {
    ...
    val expected: Result[Yaml] =
      Result.safely(Yaml.string("Mark McGwire's year was crippled by a knee injury.\n"))
  ...
  it should "correctly parse Folded newlines are preserved for \"more indented\" and blank lines" in {
    ...
    val expected: Result[Yaml] = Result.safely(
  ...
  it should "correctly print simple yaml sequences" in {
    ...
    val expected: Result[String] =
      Result.safely("""["Mark McGwire", "Sammy Sosa", "Ken Griffey"]""")

    // when
    val actual: Result[String] = yaml.printJson
  ...
  it should "correctly print simple yaml mappings" in {
    ...
    val expected: Result[String] =
      Result.safely("""{"hr": 65, "avg": 0.278, "rbi": 147}""")
  ...
  it should "correctly print mapping scalars to sequences" in {
    // given
    val expected: Result[String] =
      Result.safely("""{"american": ["Boston Red Sox", "Detroit Tigers", "New York Yankees"], "national": ["New York Mets", "Chicago Cubs", "Atlanta Braves"]}""")
  ...
  it should "correctly print sequences of mappings" in {
    // given
    val expected: Result[String] =
      Result.safely("""[{"name": "Mark McGwire", "hr": 65, "avg": 0.278}, {"name": "Sammy Sosa", "hr": 63, "avg": 0.288}]""")
  ...
  it should "return Failure for simple yaml streams" in {
    ...
    // when
    val result: Result[String] = yaml.printJson
